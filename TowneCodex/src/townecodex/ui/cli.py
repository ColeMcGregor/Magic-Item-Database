...
Implemented commands (no query/generator layer required yet):
  - import-file     : parse + (optionally) scrape and import entries
  - show            : render a single entry via the chosen renderer
  - export          : render a page of entries via the chosen renderer
  - update-price    : update an entry's price (sets value_updated=True)
  - list            : simple listing of entries

Stubs (TODO):
  - search          : will use a future QueryService
  - suggest         : will use a future SearchService (Trie)
  - run-generator   : will use a future GeneratorEngine
  - list-generators : will use a future GeneratorRepository
  - create-generator: will allow saving generator definitions
...
# used to import entries from using a given file, uses whatever parser is appropriate for the file
# #args should contain the path to the file, whether to scrape the file, and a default image to use if none is found
...
# used to show a single entry as text or HTML, if an output file is provided, it will write a full page via renderer.write_page()
# #args should contain the id of the entry to show, the renderer to use, and an optional output file
def cmd_show(args) -> int:
    ...
    if args.out:
        ...
        # Printing HTML to stdout is allowed; text renderer is nicest for terminals
        print(renderer.render_card(card))
    return 0

# used to export a page of entries using the chosen renderer, if an output file is provided, it will write a full page via renderer.write_page()
# #args should contain the renderer to use, the output file, and the title of the page
...
def cmd_list(args) -> int:
    """
    Lightweight listing (id, name, rarity, type). Not the full query layer.
    """
    repo = EntryRepository()
...
# ------------------------------------------------------------------------------
# TODO stubs (keep CLI surface stable; wire up later)
# ------------------------------------------------------------------------------

def cmd_search(_args) -> int:
    """
    TODO: implement search via a dedicated QueryService (query.py).
    For now, use 'list' or add a minimal repo-backed search if needed.
    """
